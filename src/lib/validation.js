...
export function validateMaterial(card, state, runtime) {
  ...
  if (card.category === 'rock' && card.subcategory === 'mixed_rock_soil') issues.push(issue('MATISS-0016', 'mixed_ground_source', 'warning', 'Material source represents one coherent surface', 'This source mixes rocks, soil, roots, and organic debris', ['Use it as macro ground variation or capture a cleaner rock-only source.']));
  if (card.category === 'bark') issues.push(issue('MATISS-0017', 'depth_of_field_contamination', 'warning', 'Surface source remains sharp across the material region', 'Bark source includes depth-of-field and surrounding foliage', ['Use for an organic reference or capture a flatter, evenly focused bark sample.']));
  if (card.category === 'asphalt' && card.subcategory === 'coarse_aggregate_closeup') issues.push(issue('MATISS-0018', 'macro_debris_retained', 'info', 'Macro source is free of unrelated debris', 'The close-up crop retains a small piece of organic debris from the photographed surface', ['Use the crop as a detail reference or author a cleaned replacement crop.']));
  if (card.category === 'vegetation') issues.push(issue('MATISS-0019', 'leaf_opacity_not_authored', 'warning', 'Leaf surface includes a usable opacity or cutout workflow', 'This broadleaf card is a color reference with surrounding foliage; no opacity map is authored', ['Use it on a leaf-shaped mesh or author an opacity/cutout map before using it as a leaf asset.']));
  if (card.category === 'fabric' && card.subcategory === 'wrinkled_textile') issues.push(issue('MATISS-0020', 'macro_fold_source', 'warning', 'Surface detail remains useful when tiled', 'The textile source is dominated by broad photographed folds rather than tileable micro-fiber detail', ['Use it as a cloth/decal reference or capture a flatter textile region.']));
  if (card.category === 'stone' && card.sourceFiles?.some((file) => file.role === 'alternate')) issues.push(issue('MATISS-0021', 'alternate_source_not_blended', 'info', 'Alternate source captures are aligned or intentionally compared', 'An alternate source photo is preserved but the runtime map uses one unblended capture', ['Use the alternate capture for scale comparison or author an aligned multi-source material.']));
